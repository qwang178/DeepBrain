## sample code to predict SI for TCX. CER could be predicted the same way
...
e<-read.csv(file="data/TCX.first_layer_hidden.csv",header=F)             # from pass forward trained deep learning model
dx<-read.csv(file="data/TCX.dx.csv",header=T,row.names=1)                # meta data associated with the cohort(region)                              
...
meta<-cbind(dx,umap)
meta$SI<-time
write.csv(file="data/TCX.SI.csv",meta,quote=F)
